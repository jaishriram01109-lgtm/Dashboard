...
export default function OptionsChain() {
  ...
  const liveQuotes = useAngelQuotes([symbol]);
  const liveQ = liveQuotes[symbol];
  const isLive = !!liveQ;
      ...
      <div className="card-base p-4">
        ...
        <div className="mb-2">
          ...
          <div className="flex flex-wrap gap-1.5 mt-1">
            {expiries.map((exp, i) => {
              const expDte = Math.max(0, Math.round((new Date(exp).getTime() - Date.now()) / 86_400_000));
              return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="card-base p-4">
          ...
          <div className="space-y-1.5 text-[11px] text-ivory-300">
            <p>📌 <strong>PCR {pcr.toFixed(2)}</strong> — {pcr > 1.2 ? "More puts written → bullish bias (sellers expect market to hold support)" : pcr < 0.8 ? "More calls written → bearish bias (sellers expect market to face resistance)" : "Balanced positioning — market in consolidation zone"}.</p>
            <p>🎯 <strong>Max Pain ₹{fmt(maxPain)}</strong> — {spot > maxPain ? `Spot ₹${fmt(Math.round(spot))} is ${((spot - maxPain)/spot*100).toFixed(1)}% above max pain. Expect gravitational pull downward near expiry.` : `Spot ₹${fmt(Math.round(spot))} is ${((maxPain - spot)/spot*100).toFixed(1)}% below max pain. Expect gradual drift upward near expiry.`}</p>
            <p>⏳ <strong>DTE {dte}d</strong> — {dte <= 2 ? "Expiry day / next day. Theta decay is maximum. Avoid buying options." : dte <= 7 ? "Near expiry. IV elevated, time decay accelerating. Prefer selling strategies." : "Sufficient time remains. IV relatively stable."}.</p>
          </div>
        </div>
      </div>
